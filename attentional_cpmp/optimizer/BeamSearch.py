...
class BeemSearch(OptimizerStrategy):
    def __init__(self, model: Model, data_adapter: DataAdapter, H: int, level_nodes: int, threshold: float) -> None:
        self.__model = model
        self.__H = H
    ...
    def __get_model_lays__(self, session_lays: dict) -> np.ndarray:
        ...
        for case in temp_session:
            ...
            for i in range(states_size):
                model_lays.append(self.__data_adapter.get_ann_state(states[i]))
